...
CoordinateSystem::CoordinateSystem(Protean3D::Triple first, Protean3D::Triple second, Protean3D::COORDSTYLE st)
{
...
void CoordinateSystem::setStyle(Protean3D::COORDSTYLE s, Protean3D::AXIS frame_1, 
...
void CoordinateSystem::attach(unsigned idx)
{
  if ( aidx_.end() == std::find( aidx_.begin(), aidx_.end(), idx ) )
     aidx_.push_back(idx);
}

void CoordinateSystem::detach(unsigned idx)
{
  std::list<unsigned>::iterator it = std::find(aidx_.begin(), aidx_.end(), idx);
  if ( it != aidx_.end() )
  {
    aidx_.erase(it);
  }
}
void CoordinateSystem::detachAll()
{
  aidx_.clear();
}
