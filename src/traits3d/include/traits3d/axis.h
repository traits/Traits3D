...
public:
    ...
    */
    ...
    bool isZ() const
    {
        return is_z_;   //!< Is a z-axis
    }

    void setPosition(const Triple &beg, const Triple &end); //!< Set axis position
    void position(Triple &beg, Triple &end) const
    {
        beg = axis_origin_;    //!< Returns axis' position
        end = axis_end_;
    }
    Triple begin() const
    {
        return axis_origin_;    //!< Returns axis' beginning position
    }
    Triple end() const
    {
        return axis_end_;    //!< Returns axis' ending position
    }
    double length() const
    {
        return glm::distance(axis_origin_, axis_end_);    //!< Returns axis' length
    }
    ...
    void setScale(std::shared_ptr<Scale> val)
    {
        scale_ = val;    //!< This variant sets a user-defined scale object.
    }
    void showTics(bool val)
    {
        show_tics_ = val;    //!< Turns scale drawing on or off
    }
    bool hasTics() const
    {
        return show_tics_;    //!< Returns, if scale drawing is on or off
    }
    ...
    {
        majorl = len_major_tics_;
        minorl = len_minor_tics_;
    }
    ...
    {
        return tic_orientation_;   //!< Returns tic orientation
    }
    void setSymmetricTics(bool b)
    {
        symmetric_tics_ = b;   //!< Sets two-sided tics (default is false)
    }
    void setAutoScale(bool val = true)
    {
        autoscale_ = val;    //!< Turns auto-scaling on or off
    }
    bool autoScale() const
    {
        return autoscale_;    //!< actual auto-scaling mode
    }

    //! Sets font for axis label
    void showLabel(bool d)
    {
        show_label_ = d;    //!< Turns label drawing on or off
    }
    ...
    {
        label_gap_ = val;   //!< Shifts label in device coordinates dependent on anchor;
    }

    void showNumbers(bool d)
    {
        show_numbers_ = d;   //!< Turns number drawing on or off
    }
    bool numbers() const
    {
        return show_numbers_;   //!< Returns, if number drawing is on or off
    }
    void setNumberColor(Color const &col); //!< Sets the color for axes numbers
    Color numberColor() const
    {
        return number_color_;   //!< Returns the color for axes numbers
    }
    void setNumberFont(FontInfo const &font_info); //!< Sets font for numbering
    void setNumberAnchor(TextEngine::Anchor a)
    {
        scale_number_anchor_ = a;    //!< Sets anchor position for numbers
    }
    void adjustNumbers(int val)
    {
        number_gap_ = val;   //!< Shifts axis numbers in device coordinates dependent on anchor;
    }
    ...
    size_t majors() const
    {
        return majorintervals_;    //!< Returns number of major intervals
    }
    size_t minors() const
    {
        return minorintervals_;    //!< Returns number of minor intervals
    }
    TripleVector const &majorPositions() const
    {
        return major_positions_;   //!< Returns positions for actual major tics (also if invisible)
    }
    TripleVector const &minorPositions() const
    {
        return minor_positions_;   //!< Returns positions for actual minor tics (also if invisible)
    }

    void setLimits(double start, double stop)
    {
        start_ = start;    //!< Sets interval
        stop_ = stop;
    }
    void limits(double &start, double &stop) const
    {
        start = start_;    //!< Returns axis interval
        stop = stop_;
    }
