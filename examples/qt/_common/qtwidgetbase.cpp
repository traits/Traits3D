...
void QtWidgetBase::setScaleMouse(MouseState bstate, double accel, QPoint diff)
{
    ...
    if (bstate == xscale_mstate_)
        new_xscale = std::max<double>(0.0, plot_p->xScale() + relx);

    if (bstate == yscale_mstate_)
        new_yscale = std::max<double>(0.0, plot_p->yScale() - relyz);

    if (bstate == zscale_mstate_)
        new_zscale = std::max<double>(0.0, plot_p->zScale() - relyz);

    setScale(new_xscale, new_yscale, new_zscale);

    if (bstate == zoom_mstate_)
        setZoom(std::max<double>(0.0, plot_p->zoom() - relyz));
}
...
void QtWidgetBase::wheelEvent(QWheelEvent *e)
{
    ...
    if (e->modifiers() & Qt::ShiftModifier)
        ...
        setZoom(std::max<double>(0.0, plot_p->zoom() + step));
...
*/
void QtWidgetBase::enableMouse(bool val)
{
...
\see enableMouse()
*/
void QtWidgetBase::disableMouse(bool val)
{
    mouse_input_enabled_ = !val;
}
bool QtWidgetBase::mouseEnabled() const
{
...
void QtWidgetBase::setScaleKeyboard(KeyboardState kseq, double speed)
{
    ...
    if (kseq == xscale_kstate_[0])
        new_xscale = std::max(0.0, plot_p->xScale() + relx);

    if (kseq == xscale_kstate_[1])
        new_xscale = std::max(0.0, plot_p->xScale() - relx);

    if (kseq == yscale_kstate_[0])
        new_yscale = std::max(0.0, plot_p->yScale() - relyz);

    if (kseq == yscale_kstate_[1])
        new_yscale = std::max(0.0, plot_p->yScale() + relyz);

    if (kseq == zscale_kstate_[0])
        new_zscale = std::max(0.0, plot_p->zScale() - relyz);
...
*/
void QtWidgetBase::enableKeyboard(bool val)
{
...
\see enableKeyboard()
*/
void QtWidgetBase::disableKeyboard(bool val)
{
    kbd_input_enabled_ = !val;
}
bool QtWidgetBase::keyboardEnabled() const
{
...
*/
void QtWidgetBase::setKeySpeed(double rot, double scale, double shift)
{
    if (rot > 0)
        kbd_rot_speed_ = rot;
...
void QtWidgetBase::keySpeed(double &rot, double &scale, double &shift) const
{
    rot = kbd_rot_speed_;
...
void QtWidgetBase::enableLighting(bool val /*= true*/)
{
    makeCurrent();
    plot_p->enableLighting(val);
}

void QtWidgetBase::disableLighting(bool val /*= true*/)
{
    makeCurrent();
...
void QtWidgetBase::setLightRotation(double xVal, double yVal, double zVal, unsigned int idx /*= 0 */)
{
    //plot_p->setLightRotation(xVal, yVal, zVal, idx);
    update();
}

void QtWidgetBase::setLightShift(double xVal, double yVal, double zVal, unsigned int idx /*= 0 */)
{
    //plot_p->setLightShift(xVal, yVal, zVal, idx);
    update();
}

void QtWidgetBase::paintGL()
{
    plot_p->draw();
}
...
/*!
  ...
  */
void QtWidgetBase::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
    plot_p->draw();
}

void QtWidgetBase::updateData()
{
    makeCurrent();
    plot_p->updateData();
    update();
