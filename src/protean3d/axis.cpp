...
void Axis::init()
{
  ...
  autoscale_ = true;
  markerLabel_.clear();
  numberfont_ = Protean3D::Label::Font("Courier",12);
  setLabelFont(Protean3D::Label::Font("Courier",14));
...
void Axis::drawLabel()
{
  if (!drawLabel_)
	...
	double alpha = angle(beg.x, beg.y, end.x, end.y);
...
void Axis::drawTicLabel(Triple pos, int mtic)
{
  ...
  markerLabel_[mtic].setFont(numberfont_);
...
void Axis::setNumberFont(Protean3D::Label::Font const& font)
{
  numberfont_ = font;
}
...
void Axis::setLabelFont(Protean3D::Label::Font const& font)
{
  labelfont_ = font;
  label_.setFont(labelfont_);
}

void Axis::setLabelString(Protean3D::Label::String const& name)
{
...
*/
void Axis::setLabelPosition(const Triple& pos,Protean3D::ANCHOR an)
{
