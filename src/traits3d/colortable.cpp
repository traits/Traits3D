...
Color ColorTable::color(float z, ColorVector const &vec, float zmin, float zmax)
{
    size_t len = vec.size();
...
ColorVector ColorTable::stdColor(size_t len)
{
    if (0 == len)
        return ColorVector();
...
ColorVector ColorTable::createColors(TripleVector const &data, ColorVector const &color_field)
{
...
ColorVector ColorTable::createColors(std::vector<TripleF> const &data, ColorVector const &color_field)
{
    ColorVector ret(data.size());
    Box hull = calculateBox(data);
    float zmin = static_cast<float>(hull.minVertex.z);
